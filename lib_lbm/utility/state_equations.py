"""
This file most importantly defines the StateEquations class, which represents the state equations of the district
heating network. This class is the basis for all calculations regarding the district heating network.
The most important functions are:
    - set_init_state: sets the state variables to initial values
    - set_active_edge_temperature: updates the temperature of an active edge
    - eq_pipe_T, eq_pipe_p, eq_demand_Q and eq_fix_dp; who describe the physics of the edges in the network
      (node equations are calculated directly in 'evaluate_state_equations' in matrix form)
    - evaluate_state_equations: evaluates the state equations;
        returns the residual vector and optionally gradients or the jacobi matrix
The remaining functions are helper functions, parts of the classic solvers or very specific to the implementation

The file also contains the TemperaturePropagationException, which is raised if the temperature propagation does not
reach all nodes when solving the temperatures for given mass flows (prob. a disconnected graph or invalid mass flows)

calculate_physics_violation is functionally similar to evaluate_state_equations, but is not part of the StateEquations
object for performance reasons. It is associated with a StateEquation object by the function get_phyical_loss_config.
The function is used in the PAL-Loss function to calculate the physics violation of a given state.
"""
...
class StateEquations():
    def __init__(self, edges, nodes, demands, heatings, fix_dp: dict, use_demands=True,
        ...
        A_col = []
        A_val = []
        A_active_row = []  # identity mapping matrix active massflows
        A_active_col = []
        A_active_val = []
        dem_ind = dict()
    ...
    def eq_fix_dp(self, pf, pt, p_set, lambda1=1.e-2, lambda2=1.e0):
        return lambda1 * tf.clip_by_value(pt - pf - p_set, 0, 1.e7) + lambda2 * tf.clip_by_value(pt - pf - p_set, -1.e7,
                                                                                                 0)
            ...
            for i in range(self.n_edges):
                ...
                if edg['edge control type'] == 'passive':
                    ...
                    if self.use_demands:
                        ...
                        # else:
                        #     l = T_end[i] - Q_T_end[self.dem_ind[edg['index']]]
                        if mode == 'gradient':
                            loss += tf.reduce_sum(l ** 2, axis=0)
                        else:
                            loss = tf.concat([loss, tf.expand_dims(l, axis=0)], axis=0)
    ...
    def solve_massflow_fixed_temp(self, mf_vals=None, cycles=[], alpha=1.):
        ...
        :param mf_vals: mass flow values for the active edges (optional), (ordered by self.dem_ind)
