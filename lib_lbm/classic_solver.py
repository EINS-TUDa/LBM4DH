"""
Classic (Non-AI) solver for the district heating equations using the StateEquations class

Classes:
    MaximumIterationException: Exception raised if the maximum number of iterations is reached without convergence
    my_newton_optimizer: Newton-Raphson optimizer for the StateEquations class
    my_fixpoint_iteratror: Fixpoint iterator for the StateEquations class
    SE_solver: combines the solvers above in one function to solve SE-objects
"""

import numpy as np
import tensorflow as tf
from lib_lbm.utility.state_equations import TemperatureProbagationException, StateEquations
...
class my_newton_optimizer():
    ...
    def get_desc_dir(self, jacobians, values):
        ...
        # extreme settings, e.g. if all demands are zero. ste max-iter = 100 to avoid infinite looping.
...
class SE_solver(object):
    ...
    def __call__(self, SE, cycles, max_steps=100, verbose=False, alpha=1.):
        ...
        # first run:
        loss = self.fixpoint_step(SE, cycles, alpha=alpha)
        ...
        # # detect, whether the slack is positive or negative:
        # if SE.mf[SE.find_edge[list(SE.heatings.keys())[0]]] < 0:
        #     if verbose:
        #         print('negative slack power detected, invert slack power plant')
        #     inverted = True
